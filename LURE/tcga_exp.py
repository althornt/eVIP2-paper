...
def main():
    ...
    clustermap(score_mut,tcga_exp,kegg_mapk_genes+mapkgenes+wnt_genes+mmr,"kegg_mapk_genes_andtengene" )

    lobodo_ras = open("inputs/lobodo_RAS_signature.txt","r").read().splitlines()
    clustermap(score_mut,tcga_exp,lobodo_ras,"lobodo_ras" )
    ...
    # clustermap(score_mut,tcga_exp,mapkgenes,"mapk" )
    # clustermap(score_mut,tcga_exp,popovici_genes+mapkgenes,"braf_sig_mapk" )
    # clustermap(score_mut,tcga_exp,mapkgenes+wnt+mmr,"og" )
...
# "G12D","G12S", "G12R", "G12C", "G12V", "G12A"
    ...
    sns.clustermap(tcga_exp,z_score=0,center=0,cmap="bwr",xticklabels=False,
                yticklabels=True,row_colors=gene_colors, method = "ward",
                colors_ratio = .02, figsize = (12,8),
                col_colors=[lure_scale_c,braf_status_c,rnf43_status_c,RNF43_mut_type_c,kras_status_c],
                row_cluster=True)

    plt.tight_layout()
    plt.savefig("outputs/tcga_heatmap_"+label+".png",bbox_inches = "tight",dpi=400)
    plt.savefig("outputs/tcga_heatmap_"+label+".svg",bbox_inches = "tight",dpi=400)
